...
export default function ProfileScreen() {
  ...
  const handleClear = async () => {
    ...
    const keysToRemove = [
      ...
      `wya_cel_nutrition_${user.id}`,
      `wya_cel_feeling_${user.id}`,
      `wya_cel_firstMeal_${user.id}`,
      `wya_cel_firstReflection_${user.id}`,
      `wya_dob_${user.id}`,
    ];
  ...
  return (
    <div className="min-h-screen bg-surface">
      <div className="mx-auto flex min-h-screen max-w-md flex-col px-5 pb-24 safe-top">
        ...
        <header className="mb-2" data-tour="profile-header">
          ...
          <div className="flex items-center gap-4">
            <button
              type="button"
              aria-label="Edit name and avatar color"
            ...
            >
              ...
              />
              <span className="absolute bottom-0 right-0 flex h-6 w-6 items-center justify-center rounded-full border-2 border-surface bg-white text-ink/55 shadow-sm">
        ...
        <div className="kb-avoid fixed inset-0 z-50 flex items-center justify-center bg-black/30 px-5" style={{ paddingBottom: kbInset }}>
          <div className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-lg">
            ...
            <div className="mt-3 flex justify-center">
              <ProfileAvatar firstName={editFirstName} lastName={editLastName} email={user?.email} color={avatarColor} size={64} />
            </div>
          ...
          >
            <div className="mb-6 text-center">
              ...
              {healthKitModalState === "connect" && (
                <p className="mt-1 text-xs text-muted/60">Sync steps, sleep, and workouts to make your coach smarter.</p>
              )}
            </div>
            ...
            <div>
                {healthKitModalState === "instructions" && (
                  <div>
                    {[
                      { bg: "bg-gray-500", label: "Settings", icon: (<svg className="h-4 w-4 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 1 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 1 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 1 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 1 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>) },
                      { bg: "bg-blue-500", label: "Apps", icon: (<svg className="h-4 w-4 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg>) },
                      { bg: "bg-rose-500", label: "Health", icon: (<svg className="h-4 w-4 text-white" viewBox="0 0 16 16" fill="currentColor"><path d="M8 13.7C7.7 13.5 1 9.2 1 5.5 1 3.6 2.6 2 4.5 2c1 0 2 .5 2.7 1.3L8 4.2l.8-.9C9.5 2.5 10.5 2 11.5 2 13.4 2 15 3.6 15 5.5c0 3.7-6.7 8-7 8.2z"/></svg>) },
                      { bg: "bg-teal-500", label: "Data Access & Devices", icon: (<svg className="h-4 w-4 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>) },
                      { bg: "bg-primary", label: "WhatYouAte", icon: (<svg className="h-4 w-4 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 2v7c0 1.1.9 2 2 2s2-.9 2-2V2"/><path d="M7 2v20"/><path d="M21 15V2a5 5 0 0 0-5 5v6c0 1.1.9 2 2 2h3zm0 0v7"/></svg>) },
                    ].map((step, i, arr) => (
                      <div key={i}>
                        ...
                        {i < arr.length - 1 && <div className="ml-[18px] my-1 h-3 w-px bg-ink/15" />}
                      </div>
                    ))}
                  </div>
                )}
                <div className="mt-6 space-y-2">
                  {(healthKitModalState === "connect" || healthKitModalState === "connecting") && (
                    <button
                      type="button"
                      disabled={healthKitModalState === "connecting"}
                      className="w-full rounded-xl bg-primary py-3 text-sm font-semibold text-white transition active:opacity-80 disabled:opacity-50"
                      onClick={async () => {
                        if (!user) return;
                        setHealthKitModalState("connecting");
                        await requestHealthKitPermissions();
                    ...
                    >
                      {healthKitModalState === "connecting" ? "Connecting…" : "Connect to Apple Health"}
                    </button>
                  )}
                  <button
                    type="button"
                    className="w-full rounded-xl bg-ink/8 py-3 text-sm font-semibold text-ink/70 transition active:opacity-80"
                    onClick={() => { setHealthKitShowSettings(false); setHealthKitModalState("connect"); }}
                  >
                    Got It
                  </button>
                </div>
      ...
      {showWeightHistory && (
        ...
        <div className="fixed inset-x-0 z-[51] flex items-center justify-center px-5 transition-[top,height] duration-200 ease-out" style={{ top: 0, height: kbInset ? `calc(100% - ${kbInset}px)` : "100%", paddingTop: kbInset ? "calc(env(safe-area-inset-top) + 0.5rem)" : "4vh", paddingBottom: "4vh" }} onClick={() => setShowWeightHistory(false)}>
                  ...
                  <div key={log.id} className="flex items-center justify-between rounded-lg bg-ink/[0.03] px-3 py-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="flex h-5 w-5 items-center justify-center rounded-full text-ink/30 transition hover:text-red-500 active:opacity-60"
                        onClick={() => deleteWeight(log.id)}
                      >
                        <svg viewBox="0 0 24 24" className="h-3.5 w-3.5" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M6 6l12 12M18 6L6 18" /></svg>
                      </button>
                    </div>
                  </div>
                );
