...
export default function SummaryScreen() {
  ...
  // Smart nudge — DB-only, no on-demand AI calls from the client.
  // AI nudges are generated by cron (scheduled) and HomeScreen prefetch (background).
  // SummaryScreen reads from DB only: current window first, then most recent nudge as fallback.
  useEffect(() => {
    ...
    // Try current window's nudge first
    ...
    // No current-window nudge yet — use most recent nudge from DB (any window) without loading state
    const mostRecent = nudges[0];
    if (mostRecent) {
      setSmartNudge({ message: mostRecent.message, type: mostRecent.type as NudgeType, action: mostRecent.action ?? undefined, generatedAt: mostRecent.created_at });
    } else {
      setSmartNudge(null);
    }
        ...
        <Card className={`relative mt-6${nudgeCardIsNew && smartNudge ? " ring-1 ring-primary/20" : ""}`} data-tour="nudges-card">
          ...
          {isDemoMode ? (
            ...
            <div className="mt-4 space-y-3">
              ...
              {/* Today real-time card — rule-based, instant, no loading state */}
              {!trial.isFree && visibleNotes.length > 0 && (() => {
                const todayNote = visibleNotes.find((n) => n.type !== smartNudge?.type) ?? visibleNotes[0];
                return (
                  <div>
                    <div className="flex items-center justify-between">
                      <p className="text-[11px] font-semibold uppercase tracking-wide text-muted/50">Right Now</p>
                    </div>
                    <div className="mt-1.5 rounded-xl border border-ink/10 bg-ink/3 px-4 py-3">
                      <p className="text-sm text-ink/75">{todayNote.message}</p>
                    </div>
                  </div>
                );
              })()}
