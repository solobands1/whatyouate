...
export default function SummaryScreen() {
  ...
  useEffect(() => {
    setSmartNudge(
      currentWindowNudge
        ? { message: currentWindowNudge.message, type: currentWindowNudge.type as NudgeType, action: currentWindowNudge.action ?? undefined, suggestions: (currentWindowNudge as any).suggestions ?? undefined, generatedAt: currentWindowNudge.created_at }
        : null
    );
  }, [currentWindowNudge]);
      ...
      <div className="mx-auto flex min-h-screen max-w-md flex-col px-5 pb-24 safe-top">
        ...
        <div data-tour="nutrition-reflections" className="mb-6 grid grid-cols-2 gap-3">
          <button
            ...
            className="flex flex-col items-start gap-2 rounded-2xl border border-primary/20 bg-primary/[0.05] px-4 py-3 text-left transition active:opacity-70"
          >
          ...
          <button
            type="button"
            onClick={() => router.push("/summary/reflection")}
            className="flex flex-col items-start gap-2 rounded-2xl border border-primary/20 bg-primary/[0.05] px-4 py-3 text-left transition active:opacity-70"
          >
            <span className="flex h-9 w-9 items-center justify-center rounded-full bg-primary/15 text-primary">
              <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 12.8A9 9 0 1 1 11.2 3 7 7 0 0 0 21 12.8z" /></svg>
            </span>
            <div>
          ...
          <div className="mt-3 flex items-start justify-between">
            {last7Days.map((day, i) => (
              <div key={day.key} className="flex flex-col items-center gap-1.5">
                ...
                <p className={`${day.isToday ? "text-[11px] font-bold text-ink/80" : "text-[10px] text-muted/60"}`}>{day.label}</p>
              </div>
            ))}
          </div>
