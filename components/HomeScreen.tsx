...
export default function HomeScreen() {
  ...
  useEffect(() => {
    ...
    if (prev === "active" && heroHabit.status === "dayComplete") {
      ...
      if (!isDemoMode) {
        const t = setTimeout(() => { setDailyHabitBanner(true); }, 2000);
        return () => clearTimeout(t);
      }
    }
  ...
  const greetingIcon = (phase: "morning" | "afternoon" | "evening") =>
    phase === "morning" ? (
      <svg viewBox="0 0 24 24" className="h-10 w-10" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round"><path stroke="none" fill="currentColor" fillOpacity="0.12" d="M8.1 16A4 4 0 1 1 15.9 16Z" /><path d="M12 5v2M6.1 6.9l1.2 1.6M17.9 6.9l-1.2 1.6M4.4 12.5l-1.9-0.6M19.6 12.5l1.9-0.6M8.1 16A4 4 0 1 1 15.9 16" /><path strokeWidth="1.3" d="M1 16h22M6.5 19h11" /></svg>
    ) : phase === "afternoon" ? (
      <svg viewBox="0 0 24 24" className="h-9 w-9" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="4" fill="currentColor" fillOpacity="0.12" /><path d="M12 2v2M12 20v2M4.9 4.9l1.4 1.4M17.7 17.7l1.4 1.4M2 12h2M20 12h2M6.3 17.7l-1.4 1.4M19.1 4.9l-1.4 1.4" /></svg>
    ) : (
      <svg viewBox="0 0 24 24" className="h-8 w-8" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round"><path fill="currentColor" fillOpacity="0.12" d="M21 12.8A9 9 0 1 1 11.2 3 7 7 0 0 0 21 12.8z" /></svg>
    );
  ...
  const savedMsgHandledRef = useRef<string | null>(null);
  useEffect(() => {
    if (!user) return;
    const saver = profile?.streakSaver;
    const rl = saver?.rescueLast;
    if (!rl || savedMsgHandledRef.current === rl) return;
    const y = new Date(); y.setDate(y.getDate() - 1);
    const yStr = `${y.getFullYear()}-${String(y.getMonth() + 1).padStart(2, "0")}-${String(y.getDate()).padStart(2, "0")}`;
    if (rl < yStr && saver.forgiven.includes(rl) && saver.ack !== rl) {
      savedMsgHandledRef.current = rl;
      saveStreakSaver(user.id, { ...saver, ack: rl }).catch(() => {});
      setShowSavedMessage(true);
    }
    ...
    <div className="min-h-screen bg-surface">
      ...
      <div className="relative mx-auto flex min-h-screen max-w-md flex-col px-5 pb-24 safe-top">
        ...
        {!showUpdatePopup && !showTourGate && !runTour && !isDemoMode && (showSavedMessage ? (
          ...
          <UnlockCelebrationBanner title="Back At It" sub="Good to see you moving again. Starting back up is the hard part, and you did it!" icon="spark" onDismiss={() => setComebackBanner(false)} />
        ) : firstCel ? (
            ...
            {heroHabit.status === "suggested" ? (
                ...
                <div className={`grid transition-all duration-[800ms] ease-[cubic-bezier(0.22,1,0.36,1)] ${heroExpanded ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}>
                  <div className="min-h-0 overflow-hidden">
                    ...
                    <div className="mt-2 flex justify-center">
                      <button type="button" className="text-[11px] font-medium text-ink/30 transition active:opacity-60" onClick={snoozeAll}>
              ...
              >
                ...
                <div className="flex flex-col items-center py-1">
                  <span className="mt-1.5 flex h-14 w-14 items-center justify-center rounded-full bg-primary/15 text-primary">
                    <svg viewBox="0 0 24 24" className="h-7 w-7" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="9" /><g className="animate-clock-wind"><path d="M12 7.5V12l3 1.5" /></g></svg>
                  </span>
            ...
            {backfillTarget && (
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block text-[11.5px] leading-snug text-ink/55">{backfillTarget.netAvailable ? "Finish your day · Add them all at once" : "Keep your streak · Add them all at once"}</span>
                </span>
      ...
      {showHabitSpotlight && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center px-6" role="dialog" aria-modal="true">
          ...
          >
            ...
            <p className="mt-3 text-[12px] leading-relaxed text-ink/55">This is where WhatYouAte starts learning what helps you feel your best.</p>
            <button
              type="button"
